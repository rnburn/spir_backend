...
static uint64_t encodeLLVMAttributesForBitcode(const AttributeSet &Attrs,
                                               unsigned Index) {
  uint64_t Raw = Attrs.Raw(Index);
  uint64_t EncodedAttrs = Raw & 0xffff;
  if (Attrs.hasAttribute(Index, Attribute::Alignment)) {
    Attribute Alignment = Attrs.getAttribute(Index, Attribute::Alignment);
    EncodedAttrs |= Alignment.getAlignment() << 16;
  }
  EncodedAttrs |= (Raw & (0xffffULL << 21)) << 11;
  return EncodedAttrs;
}
...
static void WriteAttributeTable(const ValueEnumerator &VE,
  ...
  for (unsigned i = 0, e = Attrs.size(); i != e; ++i) {
    ...
    for (unsigned i = 0, e = A.getNumSlots(); i != e; ++i) {
      ...
      uint64_t Encoding = encodeLLVMAttributesForBitcode(A, Index);
  ...
  for (unsigned i = FirstVal; i != LastVal; ++i) {
    ...
    if (C->isNullValue()) {
      ...
      case Instruction::ExtractElement:
        ...
        Record.push_back(VE.getTypeID(C->getOperand(0)->getType()));
        Record.push_back(VE.getValueID(C->getOperand(0)));
        // spir remove: Record.push_back(VE.getTypeID(C->getOperand(1)->getType()));
        Record.push_back(VE.getValueID(C->getOperand(1)));
        break;
      case Instruction::InsertElement:
        ...
        Record.push_back(VE.getValueID(C->getOperand(0)));
        Record.push_back(VE.getValueID(C->getOperand(1)));
        // spir remove: Record.push_back(VE.getTypeID(C->getOperand(2)->getType()));
        Record.push_back(VE.getValueID(C->getOperand(2)));
        break;
...
static void WriteInstruction(const Instruction &I, unsigned InstID,
  ...
  case Instruction::ExtractElement:
    ...
    PushValueAndType(I.getOperand(0), InstID, Vals, VE);
    pushValue(I.getOperand(1), InstID, Vals, VE);
    //spir remove: PushValueAndType(I.getOperand(1), InstID, Vals, VE);
    break;
  case Instruction::InsertElement:
    ...
    pushValue(I.getOperand(1), InstID, Vals, VE);
    pushValue(I.getOperand(2), InstID, Vals, VE);
    //spir remove: PushValueAndType(I.getOperand(2), InstID, Vals, VE);
    break;
  case Instruction::ShuffleVector:
